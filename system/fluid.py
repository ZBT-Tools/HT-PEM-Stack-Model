    ...
    def __init__(self, nx, name, species_dict=None, temp_init=298.15,
        ...
        try:
            if len(temp_init) == nx:
                ...
                raise ValueError('temp_init must be float or of length nx')
        except TypeError:
...
class GasMixture(Fluid):
    ...
    def update(self, temperature, pressure, mole_flow=None, method='ideal',
        ...
        if isinstance(self, GasMixture):
            ...
            self._concentration[:] = \
                self.calc_dry_concentration().transpose()
    ...
    def calc_molar_mass(self, mole_fraction=None):
        if mole_fraction is None:
            ...
            return np.sum(mole_fraction * self.species.mw, axis=0)
...
class TwoPhaseMixture(GasMixture):
    def __init__(self, nx, name, species_dict, mole_fractions_init,
                 liquid_props, temp_init=298.15, press_init=101325.0, **kwargs):
    ...
    def update(self, temperature, pressure, mole_flow=None, method='ideal',
        ...
        gas_conc, total_conc = self.calc_concentration(mole_flow)
        ...
        total_mw = self.calc_molar_mass(self.calc_fraction(mole_flow))
    ...
    def calc_total_specific_heat(self):
        return self.liquid_mass_fraction \
            * self.phase_change_species.liquid.specific_heat \
            + (1.0 - self.liquid_mass_fraction) * self.specific_heat
...
liquid_water_props = species.FluidProperties(1000.0, 1e-3, 4000.0, 0.2)
