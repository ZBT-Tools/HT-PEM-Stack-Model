...
I_t = 6000.  # Target currentdensity                      A/mÂ²
Fday = 96485.  # Faraday's constant                         C/mol
R = 8.3143  # ideal gas constant                        J/(kmol)
N = 20  # nodes, elements = N-1
M = 10  # Number of cell
Tu = 298.15
node_backward = matrix_database.backward_matrix_reac_flow(N)
element_backward = matrix_database.backward_matrix(N - 1)
node_forward = matrix_database.forward_matrix_reac_flow(N)
...
class Halfcell:
    def __init__(self, channel, stoi, spec_numb, val):
        ...
        self.we = full((self.channel.elements), 0.)
        self.w = full((self.channel.nodes), 0.)
        self.pe = full((self.channel.elements), self.channel.p_in)
        self.p = full((self.channel.nodes), self.channel.p_in)
        self.gamma = full((self.channel.nodes), 0.)
        self.humidity = full((self.channel.nodes), 0.)
        self.free_water = full((self.channel.nodes), 0.)
        self.i = full((self.channel.nodes), I_t)
        self.ie = full((self.channel.elements), I_t)
        for l in range(self.spec_numb):  # Init flow and con arrays (1-n)
            exec("self.q%d = full((self.channel.elements+1),0.)" % (l + 1))
            # 1 reactant(hydrogen/oxygen), 2 water, 3 (if oxygen) nitrogen
            exec("self.c%d = full((self.channel.elements+1),0.)" % (l + 1))
            # 1 reactant(hydrogen/oxygen), 2 water, 3 (if oxygen) nitrogen
            exec('self.T%d = full((self.channel.elements+1),self.channel.T_in)' % (l + 1))
    ...
    def calc_reac_flow(self):  # elementwise
        if self.channel.flow_dir == True:
            ...
            self.q1 = var1 + matmul(matrix, self.i) * self.channel.d_x / (self.val * Fday)
            self.q1e = matmul(matrix_database.node_to_element(self.channel.nodes),self.q1)

        else:
            matrix = node_backward
            var1 = self.stoi * I_t * self.channel.length / (self.val * Fday)
            self.q1 = var1 + matmul(matrix, self.i) * self.channel.d_x / (self.val * Fday)
            self.q1e = matmul(matrix_database.node_to_element(self.channel.nodes), self.q1)
    ...
    def calc_water_flow(self):  # water molar flux in the channel#elementwise
        if self.pem_type is True:
            if self.channel.flow_dir == True:
                matrix = node_forward
                a = self.channel.d_x / (self.val * Fday * 0.5) * matmul(-matrix, self.i)
                # production
                b = saturation_pressure_vapour.water.calc_psat(self.channel.T_in)*self.channel.phi #richtig?
                c = (0.79 / 0.21 + 1.) * self.q1[0] * b / (self.channel.p_in - b)  # Value at q2[0]
                self.q2 = a + c
                self.q2e = matmul(matrix_database.node_to_element(self.channel.nodes),self.q2)
                self.q3 = full((self.channel.nodes), self.q1[0] * 0.79 / 0.21)
                self.q3e = full((self.channel.elements), self.q1[0] * 0.79 / 0.21)

            elif self.channel.flow_dir == False:
                matrix = node_backward
                b = saturation_pressure_vapour.water.calc_psat(self.channel.T_in)*self.channel.phi
                self.q2 = full(self.channel.nodes, self.q1[-1] * b / (self.channel.p_in - b))
                self.q2e = full(self.channel.elements, self.q1[-1] * b / (self.channel.p_in - b))
        else:
            if self.channel.flow_dir == True:
                start_cell = 0
                matrix = node_forward
                ...
                b = saturation_pressure_vapour.water.calc_psat(self.channel.T_in)*self.channel.phi
                c = (0.79 / 0.21 + 1.) * self.q1[0] * b / (self.channel.p_in - b)  # Value at q2[0]
                d = reshape(self.channel.d_x * matmul(matrix, self.j), self.channel.nodes)
                # crossover
                e = a + c + d
                self.q2e = matmul(matrix_database.node_to_element(self.channel.nodes), self.q2)
                self.q3 = full((self.channel.nodes), self.q1[0] * 0.79 / 0.21)
                ...
                matrix = node_backward
                b = saturation_pressure_vapour.water.calc_psat(self.channel.T_in)*self.channel.phi
                c = self.q1[-1] * b / (self.channel.p_in - b)
                e = self.channel.d_x * matmul(-matrix, self.j)
                self.q2 = e + c
                self.q2e = matmul(matrix_database.node_to_element(self.channel.nodes), self.q2)
    ...
    def calc_pressure(self):  # channel pressure #qo qc qn / qh qa#elementwise
        ...
        if self.spec_numb == 3:
            c = self.q1 + self.q2 + self.q3 - self.w
            d = R * self.T1 / self.p
            self.p = a - b * matmul(-node_forward, c * d)
            self.pe = matmul(matrix_database.node_to_element(self.channel.nodes), self.p)

        else:
            c = self.q1 + self.q2 - self.w
            d = R * self.T1 / self.p
            self.p = a - b * matmul(-node_backward, c * d)
            self.pe = matmul(matrix_database.node_to_element(self.channel.nodes), self.p)
        ...
        if self.spec_numb == 3:  ###cathode
            ...
            for w in range(self.channel.nodes):
                ...
                if saturation_pressure_vapour.water.calc_psat(self.T2[w]) / (R * self.T2[w]) <= self.c2[w]:
                    ...
                    self.c1[w] = var3 * var4
        self.c1e = matmul(node_to_element(self.channel.nodes), self.c1)
        self.c2e = matmul(node_to_element(self.channel.nodes), self.c2)

    def calc_con_ic(self):  # Gas cocentrations in the channel [moles/mÂ³]#nodewise
        # no liquid water
        if self.spec_numb == 3:  ###cathode
            for w in range(self.channel.nodes):
                a = (self.p[w] - saturation_pressure_vapour.water.calc_psat(self.T2[w])) / (R * self.T2[w])
    ...
    def calc_cond_rates(self):  # condensation rates of the vapour in the channel#nodewise
        if self.spec_numb is 3:
            self.gamma = gradient(self.w, self.channel.d_x)
        else:
            self.gamma = -gradient(self.w, self.channel.d_x)
...
class Cell:
    ...
    def __init__(self, anode, cathode, Gamma, alpha, mem_thic, mu_p, mu_g, mu_m, e0, c_ref, i_ref, delta, T_cool_in,
                 pem_type):
        self.anode = anode
        ...
        self.pem_type = pem_type
        self.j = full(self.cathode.channel.elements + 1, 0.)
        self.Ke = 0.
        self.Ho = -52300.
        self.Ko = 6.2
        self.m_a = full((self.cathode.channel.elements + 1), 0.)
        self.m_c = full((self.cathode.channel.elements + 1), 0.)
        self.m0 = full((self.cathode.channel.elements + 1), 0.)
        self.m1 = full((self.cathode.channel.elements + 1), 0.)
        self.m_pos0 = full((self.cathode.channel.elements + 1), 0.)
        self.m_pos1 = full((self.cathode.channel.elements + 1), 0.)
        self.omega = full((self.cathode.channel.elements + 1), 0.)
        self.psi = full((self.cathode.channel.elements + 1), 0.)
        self.T = full(self.cathode.channel.elements + 1, self.T_cool_in)
        self.Te = full(self.cathode.channel.elements, self.T_cool_in)
    ...
    def calc_mem_block_1(self):  # nodewise
        ...
        b = reshape(self.i / (2. * self.Gamma * self.alpha * Fday), self.cathode.channel.nodes)
        self.zeta_plus = a + b
        c = self.cathode.free_water - self.anode.free_water
        d = reshape(5. * self.i / (2. * self.Gamma * self.alpha * Fday), self.cathode.channel.nodes)
    ...
    def calc_cross_over_water_flux(self):  # nodewise
        a = reshape(self.i / Fday, self.cathode.channel.nodes)
...
class Stack:
    ...
    def __init__(self, cell, cell_numb):
        ...
        self.i = full((cell_numb, cell.cathode.channel.nodes), I_t)
        self.T = full((cell_numb, cell.cathode.channel.nodes), self.cell.T_cool_in)
        self.Te = full((cell_numb, cell.cathode.channel.elements), self.cell.T_cool_in)
        self.g = 0.  # coolant flow
        self.h_vap = 45400.  # vaporization heat
        if self.cell.pem_type is False:
    ...
    def calc_T3(self):
        for q, item in enumerate(self.cell_list):
            ...
            var5 = self.cell.mu_g + self.cell.mu_m - self.cell.mu_m ** 2. / (
                        self.cell.mu_g + self.cell.mu_m - self.cell.mu_g ** 2. / (self.cell.mu_p + self.cell.mu_g))
    ...
    def calc_T5(self):
        for q, item in enumerate(self.cell_list):
            ...
            self.cell_list[q].T5 = (f4 + var1 + self.cell.mu_g * self.cell_list[q].T4) / (
                        self.cell.mu_p + self.cell.mu_g)
            # print(self.cell_list[q].T5,'T5')

    def calc_layer_T(self):
        # for q, item in enumerate(self.cell_list):
        # self.cell_list[q].T1 = full((self.cell.cathode.channel.elements+1), 300.)
        # self.cell_list[q].T2 = full((self.cell.cathode.channel.elements+1), 300.)
        # self.cell_list[q].T3 = full((self.cell.cathode.channel.elements+1), 300.)
        # self.cell_list[q].T4 = full((self.cell.cathode.channel.elements+1), 300.)
        # self.cell_list[q].T5 = full((self.cell.cathode.channel.elements+1), 300.)
        self.calc_T3()
        self.calc_T2()
        self.calc_T1()
        self.calc_T4()
...
class Simulation():
    ...
    def __init__(self, stack, k_it, max_it):
        ...
        self.max_it = max_it
        self.i = self.stack.i
        self.b = matrix_database.b(self.stack.cell.cathode.channel.nodes, self.stack.cell_numb,
                                   self.stack.cell.cathode.channel.length / (
                                               self.stack.cell.cathode.channel.nodes))
        self.c = matrix_database.c_new(self.stack.cell.cathode.channel.nodes, self.stack.cell_numb,
                                   self.stack.cell.cathode.channel.length / (
                                               self.stack.cell.cathode.channel.nodes))
    ...
    def calc_initial_current_density_fsolve(self, x, i, w, v):
        ...
        c = log(x * self.stack.cell.c_ref / (
                    self.stack.cell.i_ref * (self.stack.cell_list[i].cathode.c1[w] - self.stack.cell.delta * x)))
        return a - b * c - v

    def calc_initial_current_density(self):
        a = []
        for i in range(self.stack.cell_numb):
            avv = sum(self.stack.cell_list[i].v) / (self.stack.cell.cathode.channel.nodes)
            b = []
    ...
    def calc_n(self,i):#electrical coupling on
        self.stack.set_i(toarray(i))# 1d/2d? aktuell 2d
        #var0 = I_t * (self.stack.cell.cathode.channel.nodes)
        #self.stack.i[int(M/2), -1] = var0 - sum(self.stack.i[int(M/2)]) + self.stack.i[int(M/2), 0]
        self.stack.update()
        var1 = matmul(self.b, self.stack.v)
        var2 = self.stack.resistance * matmul(self.c, self.stack.i.flatten(order='C'))
        return var1 - var2

    def calc_no_ec(self,i):
        self.stack.set_i(toarray(i))
        self.stack.update()
        self.calc_initial_current_density()
        return self.stack.i.flatten(order='C')

    def calc_i_no_ec(self):
        self.calc_no_ec(self.stack.i)
        #self.i = toarray(newton_krylov(self.calc_no_ec,self.stack.i.flatten(order='C'), maxiter = 10))
        #self.i = toarray(fsolve(self.calc_no_ec,self.stack.i.flatten(order='C')))

    def calc_i(self):
        #istack = newton_krylov(self.calc_n,self.stack.i.flatten(order='C'),maxiter = 100)
        fsolve(self.calc_n,self.stack.i.flatten(order='C'),xtol=10.**-4,maxfev = 100)


channel_anode = Channel(0.67, N, 20. * 10. ** 3., 3. * 10. ** 5., 298.15,0.5, False)
anode = Halfcell(channel_anode, 1.3, 2, 2.)
channel_cathode = Channel(0.67, N, 20. * 10. ** 3., 3.2 * 10. ** 5., 298.15,0.5, True)
cathode = Halfcell(channel_cathode, 1.3, 3, 4.)
cell = Cell(anode, cathode, 0.62 * 10. ** -5., 1200., 50. * 10 ** -6, 2300., 5000., 11200., 0.944, 40.9, 64.,
            0.8 * 10. ** -3., 293., pem_type)
stack = Stack(cell, M)
simulation = Simulation(stack, 1. * 10. ** -3., 10.)
simulation.update()

print(sum(simulation.stack.i/(N*M)))
