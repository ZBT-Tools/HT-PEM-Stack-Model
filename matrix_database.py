...
def t_mat_no_bc_col(nodes, cells, r_g, r_m, r_p, r_gp, r_gm, r_pp,
    ...
    if cells is 1:
        ...
        diag_vec_mid = np.array([1. / r_g[0] + 1. / r_ano[0] + 1. / r_conv_gp[0] + 1. / r_col[0],
                                 ...
                                 1. / r_g[0] + 1. / r_m[0] + 1. / r_conv_gm[0],
                                 1. / r_g[0] + 1. / r_p[0] + 1. / r_cat[0] + 1. / r_conv_gp[0],
                                 -1. / r_p[0] - 1. / r_col[0] - 1. / r_conv_pp[0]])
        diag_vec_bc = np.array([1. / r_g[0] + 1. / r_ano[0] + 1. / r_conv_gp[0] + 2. / r_conv_e_gp[0] + 1. / r_col[0],
        ...
        for q in range(cells):
            ...
            node_r_cell_l_side = np.block(np.hstack((np.tile(node_r, nodes - 1), zero)))
            node_r_cell_mid = -block_diag(*np.hstack((node_r, 2. * np.tile(node_r, nodes - 2), node_r)))
            node_r_cell_r_side = np.block(np.hstack((np.tile(node_r, nodes - 1), zero)))
            if q is 0:
                ...
                diag_vec_bc = np.array(
                    [1. / r_p[q] + 1. / r_g[q] + 1. / r_ano[q] + 1. / r_conv_gp[q] + 2. / r_conv_e_gp[q],
                ...
                diag_vec_bc = np.array([1. / r_g[q] + 1. / r_ano[q] + 1. / r_conv_gp[q] + 2. / r_conv_e_gp[q],
                                        1. / r_g[q] + 1. / r_m[q] + 1. / r_conv_gm[q] + 2. / r_conv_egm[q],
                                        1. / r_g[q] + 1. / r_m[q] + 1. / r_conv_gm[q] + 2. / r_conv_egm[q],
                                        1. / r_g[q] + 1. / r_p[q] + 1. / r_cat[q] + 1. / r_conv_gp[q] + 2. /
                                        r_conv_e_gp[q],
            ...
            diag_vec_cell = block_diag(
                *np.hstack((diag_vec_bc * 0.5, np.tile(diag_vec_mid, nodes - 2), diag_vec_bc * .5)))
