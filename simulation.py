...
class Simulation:
    ...
    def plot_cell_var(self, path, y_var, y_label, x_label,
        ...
        for i, item in enumerate(self.stack.cells):
            plt.plot(x_var, eval('self.stack.cells' +
                                 '['+str(i)+']'+'.' + y_var),
                     color=plt.cm.coolwarm(i / self.stack.cell_numb),
                     marker='.')
    ...
    def output_plots(self, case_num):
        ...
        """
        path = os.path.join(self.output_dir, 'case' + case_num, 'plots')
        if os.path.exists(path):
            shutil.rmtree(path)
        os.makedirs(path)
        ch_length = ch_dict.dict_cathode_channel['channel_length']
        x_node = np.linspace(0., ch_length,
        ...
        if self.stack.cell_numb > 1:
            ...
            g_func.output([self.stack.manifold[0].cell_stoi/2.5],
                          ...
                          path)
        self.plot_cell_var(path, 'v', 'Voltage $[V]$', 'Channel Location $[m]$',
                           'linear', 'Cell Voltage',
                           [0., ch_length],
                           x_ele, [0.0, 1.0])
        ...
        plt.autoscale(tight=True, axis='both', enable=True)
        plt.tight_layout()
        plt.savefig(os.path.join(path+'Z-Cut-Temperature.png'),
                    format='png')
        plt.close()
    ...
    def output_csv(self, q):
        path = os.path.join(self.output_dir, 'case' + q, 'csv_data')
        if os.path.exists(path):
            shutil.rmtree(path)
        os.makedirs(path)
        for i, item in enumerate(self.stack.cells):
